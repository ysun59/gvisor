...
func (e *endpoint) write(p tcpip.Payloader, opts tcpip.WriteOptions) (int64, tcpip.Error) {
	...
	info := u.ctx.PacketInfo()
	pkt := stack.NewPacketBuffer(stack.PacketBufferOptions{
		ReserveHeaderBytes: header.UDPMinimumSize + int(info.MaxHeaderLength),
	...
	if info.RequiresTXTransportChecksum &&
		(!e.ops.GetNoChecksum() || info.NetProto == header.IPv6ProtocolNumber) {
		udp.SetChecksum(^udp.CalculateChecksum(header.ChecksumCombine(
			header.PseudoHeaderChecksum(ProtocolNumber, info.LocalAddress, info.RemoteAddress, length),
			pkt.Data().AsRange().Checksum(),
		)))
	}
	if err := u.ctx.WritePacket(pkt, false /* headerIncluded */); err != nil {
