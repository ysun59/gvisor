...
const (
	...
	// This is always communicated using FirstProtocolVersion. lisafs requires
	// that the client makes a successful Mount RPC before making other RPCs.
	Mount MID = 1
...
func MaxMessageSize() uint32 {
	...
	// size, HugePageSize is allocated and can be backed by a single THP
	// (transparent huge page) if supported by the underlying memfd.
	return hugePageSize() - uint32(os.Getpagesize())
}

func hugePageSize() uint32 {
	// Use hostarch value as a backup. Prefer the value returned by the host.
	hps := uint32(hostarch.HugePageSize)
	bs, err := ioutil.ReadFile("/sys/kernel/mm/transparent_hugepage/hpage_pmd_size")
	if err != nil {
		return hps
	}
	hostHPS, err := strconv.Atoi(strings.TrimSuffix(string(bs), "\n"))
	if err != nil {
		return hps
	}
	return uint32(hostHPS)
}
...
// NoopMarshal a noop implementation of marshal.Marshallable.MarshalBytes.
func NoopMarshal([]byte) {}

// NoopUnmarshal a noop implementation of marshal.Marshallable.UnmarshalBytes.
